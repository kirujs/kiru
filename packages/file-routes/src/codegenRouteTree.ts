...
export function codegenRouteTree(
  ...
  const emitPage = (node: FileRouteDirNode): string => {
    ...
    if (configSpread.length > 0) {
      routeDeclarators.push(
        [
          `${varName} = createRoute(${pathLiteral}, {`,
          ...configSpread.map((l) => `    ${l.trim()}`),
      ...
      routeDeclarators.push(
        `${varName} = createRoute(${pathLiteral}, () => import(${JSON.stringify(importPath)}))`
      )
    }
    return varName
  }
  ...
  const emitScope = (node: FileRouteDirNode): string => {
    ...
    routeDeclarators.push(
      [
        `${varName} = createRouteScope({`,
        ...configLines.map((l) => `    ${l.trim()}`),
  ...
  if (augmentRouteTree && (leafRouteVars.length > 0 || extendImportPath)) {
  ...
  const routerImports = [
    ...
    "createRouteTree",
    ...(middlewareImports.length > 0 ? ["collectRouteMiddlewareModule"] : []),
    ...(configImports.length > 0 ? ["resolveRouteConfig"] : []),
  ]

  const exportLine =
    leafRoutePaths.length > 0
      ? "export { routes, type PageRoute }"
      : "export { routes }"

  const pageRouteTypeLines =
    leafRoutePaths.length > 0
      ? [
          "type PageRoute =",
          ...leafRoutePaths.map((p) => `  | ${JSON.stringify(p)}`),
          "",
        ]
      : []

  const routesDeclarator = [
    "routes = createRouteTree({",
    ...rootTreeLines.map((l) => `    ${l.trim()}`),
    "  })",
  ].join("\n")

  const routeDeclBlock =
    routeDeclarators.length > 0
      ? [
          `const ${routeDeclarators[0]},`,
          ...routeDeclarators.slice(1).map((d) => `  ${d},`),
          `  ${routesDeclarator}`,
        ].join("\n")
      : `const ${routesDeclarator}`
