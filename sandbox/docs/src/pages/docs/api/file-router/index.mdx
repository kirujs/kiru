...
<Head.Content>
  ...
  <meta
    name="keywords"
    content="filerouter, router, route, link, useFileRouter, SSG, static site generation"
  />
  <meta
    name="description"
    content="Learn how to use FileRouter in Kiru for file-based routing in SPAs and SSG with automatic route generation, layouts, dynamic routes, client-side navigation, and static site generation."
  />
</Head.Content>
...
<CalloutBlock>
  Caching is ignored for `static` loaders since data is baked into the build output.
</CalloutBlock>
...
3. **Data Loading**: Loaders with `static:true` are executed during build to fetch data
...
export const config = definePageConfig({
  loader: {
    statuc: true, // bake data into the page at build time
    load: async ({ params }) => {
...
<CalloutBlock variant="info">
  During SSG, your loader functions that have `static: true` will be called for
  each route that needs to be pre-rendered. Make sure your data fetching logic
  can handle being called multiple times during the build process. When fetching
  data from a rate-limited API, use the `ssg.build.maxConcurrentRenders` plugin
  option to limit the number of concurrent requests during the build process.
</CalloutBlock>
...
<CalloutBlock>
  In CSR deployments without a top-level `404` page, deep links will load the
  app shell and you should display a client-side 404 when no route matches. In
  SSG builds, ensure `pages/404/index.tsx` exists so a static 404 is emitted.
</CalloutBlock>
