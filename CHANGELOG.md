...
- **Link prefetch:** `prefetch={{ trigger, chunks, data }}` with context-gate safety and optional `/?loader=` data warmup via `kiru/router/loaderClient`; hover on `pointerenter` + `mouseenter`; `serverLoader` uses client cache for prefetch + click dedupe when `staleTime: 0`.
