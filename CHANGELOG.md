...
- **Link prefetch:** `prefetch={{ trigger, chunks, data }}` with context-gate safety and optional `/?loader=` data warmup via `kiru/router/loaderClient`.
- **Refactors:** `clientRoutePrep.ts`, `buildLoaderContextForMatch`, `rendererStream.ts`, `resolveSsrRouteModule`; router internals moved to `routerRuntime.ts` (no public `__*` fields).
